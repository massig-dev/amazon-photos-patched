...
class AmazonPhotos:
    ...
    def determine_tld(self, cookies: dict) -> str:
        ...
        for k, v in cookies.items():
            ...
            if k.startswith(x := 'at-acb'):
                tld = k.split(x)[-1]
                # 日本アカウント: 'jp' → 'co.jp'
                return 'co.jp' if tld == 'jp' else tld
        logger.warning('TLD を判定できません。デフォルト: com')
        return 'com'
    ...
    def usage(self) -> dict | pd.DataFrame:
        ...
        if r is None:
            raise ConnectionError('usage API: max retries exceeded')
        data = r.json()
        data.pop('lastCalculated', None)
        return pd.DataFrame(
            {
                'type': k,
    ...
    def get_root(self) -> dict:
        ...
        if r is None:
            raise ConnectionError('get_root: max retries exceeded')
        data = r.json().get('data', [])
        if not data:
            raise ValueError('get_root: no root node found')
        root = data[0]
    ...
    def get_folders(self) -> list[dict]:
        """
        Get all folders in Amazon Photos (sync version for Python 3.14 compat)
        ...
        while queue:
            ...
            try:
                ...
                logger.warning(f'get_folders error: {e}')
        return folders
